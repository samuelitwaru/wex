...
heading_style = ParagraphStyle(
    name='heading', fontSize=20, wordWrap=True, alignment=1, padding=20)
heading_style2 = ParagraphStyle(
    name='heading', fontSize=15, wordWrap=True, alignment=1, padding=20)
title_style = ParagraphStyle(
    name='title', fontSize=10, wordWrap=True, alignment=1)

space = Spacer(0, 0.2*inch)
hr = HRFlowable(width=560, thickness=2, color=colors.black,
...
class PDFReport:
    ...
    def create_elements(self):
        ...
        for element in [entity_table, hr, student_table, space, body_table, space]:
    ...
    def run(self):
        ...
        doc.build(Story, onFirstPage=insert_water_mark,
...
def create_activity_body_table(computed_report, columns):
    ...
    for subject_report in subject_reports:
        ...
        cols1 = [{'col': 'code', 'name': 'code'},
                 {'col': 'subject', 'name': 'name'}]
        cols2 = [{'col': 'competency', 'name': 'name'}, {
            'col': 'score', 'name': 'score'}, {'col': 'descriptor', 'name': 'descriptor'}]
        cols3 = [{'col': 'skills', 'name': 'skills'}, {'col': 'remarks', 'name': 'remarks'}, {
            'col': 'subjectTeacher', 'name': 'subject_teacher_initials'}]
        row = []
        ...
        try:
            ...
            [row.append(getattr(activity1, col['name']))
             for col in cols2 if columns.get(col['col'])]
        except IndexError:
...
def create_assessment_body_table(computed_report, columns):
    ...
    for subject_report in subject_reports:
        ...
        cols1 = [{'col': 'code', 'name': 'code'},
                 {'col': 'subject', 'name': 'name'}]
        cols2 = [{'col': 'paper', 'name': 'description'}, {'col': 'assessments', 'name': 'scores_string'}, {'col': 'score', 'name': 'score'}, {
            'col': 'descriptor', 'name': 'descriptor'}, {'col': 'total', 'name': 'total'}, {'col': 'average', 'name': 'average'}]
        cols3 = [
            {'col': 'subjectAverage', 'name': 'average'}, {'col': 'aggregates',
                                                           'name': 'aggregate'}, {'col': 'grade', 'name': 'letter_grade'},
        ...
        try:
            ...
            [row.append(str(getattr(paper1, col['name'])))
             for col in cols2 if columns.get(col['col'])]
        except IndexError:
            [row.append('') for col in cols2 if columns.get(col['col'])]
    ...
    def __init__(self, scores_qs):
        ...
        try:
            ...
            self.title = Paragraph(
                f'Assessment: {self.assessment}', heading_style)
