...
class Entity(models.Model):
    id = models.IntegerField(primary_key=True, default=1,
                             validators=SINGLE_ENTRY_VALIDATOR)
    name = models.CharField(max_length=128)
    location = models.CharField(max_length=256)
    telephone = models.CharField(max_length=16, null=True, blank=True)
    email = models.EmailField(null=True, blank=True)
    logo = ResizedImageField(upload_to=entity_logo_upload_loacation,
                             storage=OverwiteStorageSystem, null=True, blank=True)
...
class Profile(TimeStampedModel):
    ...
    signature = ResizedImageField(
        upload_to=user_signature_upload_location, storage=OverwiteStorageSystem, null=True, blank=True)
